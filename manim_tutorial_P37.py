...
class ExampleApproximation(GraphScene):
    ...
    def construct(self):
        ...
        for text in term_num:
            text.to_edge(BOTTOM, buff=SMALL_BUFF)

        polynomials = [
            TextMobject("test" + str(n))
            for n in range(0, 6)
        ]

        for poly in polynomials:
            poly.to_edge(TOP+LEFT_SIDE, buff=SMALL_BUFF)

        #term = TexMobject("")
        #term.to_edge(BOTTOM,buff=SMALL_BUFF)

        term = VectorizedPoint(BOTTOM)
        poly = VectorizedPoint(TOP+LEFT_SIDE)
